...
async function runCodexProof() {
  ...
  const lines = [
    ...
    `Context:      ${context?.status ?? "missing"} ${context?.latencyMs ?? "?"}ms, status=${contextDetails?.contextStatus ?? "unknown"}, blocks=${contextDetails?.blocks ?? "?"}, tokens=${contextDetails?.tokens ?? "?"}`,
    `Context load: fallback=${contextDetails?.fallback ?? "none"}, pressure=${contextDetails?.pressureReason ?? "none"}, runtime=${contextDetails?.runtimeStatus ?? "unknown"}, observe=${(contextDetails?.observeCapture as Record<string, unknown> | undefined)?.status ?? contextDetails?.observeCaptureStatus ?? "unknown"}`,
