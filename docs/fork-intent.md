...
| Codex-native integration | Clarify and support a host-specific adapter path where a Codex fork posts lifecycle events directly to `agentmemory`, rather than using only MCP. | [`docs/codex_followups_spec.md`](./codex_followups_spec.md), [`README.md`](../README.md), [`src/triggers/api.ts`](../src/triggers/api.ts), [`src/types.ts`](../src/types.ts), [`test/codex-compat.test.ts`](../test/codex-compat.test.ts) |
